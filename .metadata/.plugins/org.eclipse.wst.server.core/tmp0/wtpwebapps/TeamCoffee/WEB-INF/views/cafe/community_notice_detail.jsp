...
<div class="btnDiv">
	...
	<input type="button" class="btn btn-secondary btns" value="목 록" onclick="location.href='/cafe/community1'">
	...
	<c:if test="${not empty sessionScope.memberLoginInfo or not empty sessionScope.ownerLoginInfo}">
		<c:choose>
			<c:when test="${sessionScope.memberLoginInfo.memId eq 'admin' or sessionScope.ownerLoginInfo.ownerId eq 'admin' }">
				...
				<input type="button" id="delete" class="btn btn-secondary btns" data-boardNum="${board.boardNum }" data-boardCode="${board.boardCode }" value="삭 제">
			</c:when>
		</c:choose>
	</c:if>	
	</form>
