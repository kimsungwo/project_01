...
(function($){
	menuInfoModal = function(menuCode) {
		$.ajax({
            ...
            $('.totalPrice').text(result.menuPrice.toLocaleString());
    		...
    		$.ajax({
                ...
                success: function(result) {
                   //ajax 실행 성공 후 실행할 코드 작성
                	...
                	let str = '';
                	if(result.length == 0){
                		str += '<div id="noticeDiv">';
                    	str += '<h4>등록된 메뉴가 없습니다.</h4>';
                    	str += '</div>';
                	}
                	else{
