...
//		        }
	...
	//아이디 중복확인
	ownerIdCheck = function(){
		const ownerId = $('#inputId').val();
		if(ownerId == '') {
			alert('아이디를 입력 해 주세요');
			return;
		}
		$.ajax({
            url: '/owner/ownerIdCheck', //요청경로
            type: 'post',
            data:{'ownerId' : ownerId}, //필요한 데이터
	...
	//이메일 중복체크
	ownerEmailCheck = function() {
		const ownerEmail = $('#inputEmail').val();
		if(ownerEmail == '') {
			alert('이메일을 입력 해 주세요');
			return;
		}
		$.ajax({
            url: '/owner/ownerEmailCheck', //요청경로
            type: 'post',
            data:{'ownerEmail' : ownerEmail}, //필요한 데이터
            success: function(result) {
               //ajax 실행 성공 후 실행할 코드 작성
               if(result != ''){
            	   alert('이미 등록된 이메일입니다');
